...
class Functions {
  ...
  public static function GetShop() {
    ...
    // Get DB connection from Database class (mysqli)
    $mysqli = Database::GetInstance();

    // Query active items
    $query = $mysqli->query("SELECT * FROM shop_items WHERE active = 1");
    ...
    foreach ($rows as $row) {
        ...
        foreach ($row as $key => $value) {
            ...
            // convert tinyints to booleans
            if ($key === 'active' || $key === 'amount') {
                $value = (bool)$value;
            }
